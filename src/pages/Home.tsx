      ...
      {!selectedField && (
        <div className="max-w-6xl mx-auto mb-12 animate-slide-up" style={{ animationDelay: '0.2s' }}>
          <div className="flex flex-col md:flex-row gap-6">
            <button onClick={() => setIsAiModalOpen(true)} className="flex-1 bg-white/80 backdrop-blur-xl border border-white/60 p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all group text-left relative">
              <div className="absolute inset-0 bg-gradient-to-r from-purple-500/10 to-indigo-500/10 opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl"></div>
              <div className="relative z-10 flex justify-between items-center">
                ...
                <div className="bg-purple-100 p-3 rounded-full text-purple-600 group-hover:scale-110 transition-transform"><Bot size={24} /></div>
              </div>
            </button>

            <button onClick={() => setIsQuizOpen(true)} className="flex-1 bg-white/80 backdrop-blur-xl border border-white/60 p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all group text-left relative">
              ...
              <div className="relative z-10 flex justify-between items-center">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <div className="relative">
                      <div onClick={(e) => { e.stopPropagation(); setShowDnaTooltip(!showDnaTooltip); }} className="cursor-pointer text-slate-400 hover:text-cyan-600 transition-colors p-1 rounded-full hover:bg-slate-100"><Info size={14} /></div>
                      {showDnaTooltip && (
                ...
                <div className="bg-cyan-100 p-3 rounded-full text-cyan-600 group-hover:scale-110 transition-transform"><Search size={24} /></div>
              </div>
            </button>
          </div>
        </div>
            ...
            <div className="p-8">
              {isGenerating ? (
                ...
                <div className="space-y-4">
                  ...
                  <textarea value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} className="w-full h-32 p-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none resize-none font-body text-slate-900" placeholder="e.g. A subscription service for organic dog treats..." />
                  <div className="flex justify-end pt-4">
                    <button onClick={submitAiPrompt} disabled={!aiPrompt.trim()} className={`px-8 py-3 rounded-lg font-bold flex items-center gap-2 transition-all ${!aiPrompt.trim() ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-200'}`}>
                ...
                <div className="p-8">
                  ...
                  <div className="space-y-3">
                    {FOUNDER_QUIZ[quizStep].options.map((opt) => (
                      <button key={opt.value} onClick={() => handleQuizAnswer(opt.value)} className="w-full p-4 border border-slate-200 rounded-xl hover:bg-cyan-50 hover:border-cyan-300 text-left transition-all font-body text-slate-700 font-medium hover:shadow-sm">
                        {opt.label}
                      </button>
                    ))}
                  </div>
                ...
                <div className="p-8">
                  ...
                  <div className="space-y-4">
                    {quizResult.map((biz, index) => (
                      <div key={biz.id} className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex items-center justify-between group hover:border-emerald-300 transition-colors shadow-sm">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center font-bold font-display">#{index + 1}</div>
                        ...
                        <button
                          ...
                          className="px-4 py-2 bg-slate-900 text-white rounded-lg font-bold hover:bg-emerald-600 transition-colors shadow-md text-sm font-body flex items-center gap-2"
                        >
