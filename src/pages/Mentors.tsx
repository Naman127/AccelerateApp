...
export const Mentors = ({ 
  ...
  const displayMentors = globalSearch 
    ? mentors.filter(m => 
        m.name?.toLowerCase().includes(globalSearch.toLowerCase()) || 
        m.expertise?.toLowerCase().includes(globalSearch.toLowerCase()) ||
        m.role?.toLowerCase().includes(globalSearch.toLowerCase()) ||
        m.bio?.toLowerCase().includes(globalSearch.toLowerCase())
      )
    : mentors;
    ...
    <div className="max-w-6xl mx-auto animate-fade-in relative z-10 pb-20">
      ...
      {displayMentors.length === 0 ? (
        <div className="text-center py-20 bg-white/40 backdrop-blur-md rounded-3xl border border-dashed border-slate-300">
          ...
          <h3 className="text-xl font-bold text-slate-700 font-display">No mentors found</h3>
          <p className="text-slate-500 font-body">Try adjusting your search terms.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayMentors.map((mentor) => {
            ...
            return (
              <div 
                key={mentor.id} 
                className={`bg-white/80 backdrop-blur-xl border border-white/60 rounded-3xl p-6 shadow-sm hover:shadow-lg hover:border-indigo-300 transition-all flex flex-col text-center group ${globalSearch ? 'search-match' : ''}`}
              >
